...
int Server::start(bool fork) {
	...
	while (true) {
		...
		if (diffMillis <= SELECT_LOG_FAST_LOOP) {
			LOG(Logger::BULK, "returned from select() after %li ms", diffMillis);
		}
		...
		for (vec_ClientP::iterator it = clients_.begin(); it != clients_.end(); /* increment inside loop */) {
			...
			if (rv >= 0 || (rv == -2 && client->getBufferSize() > 0)) {
				if (rv >= 0) LOG(Logger::BULK, "read %i bytes from client with fd %i", rv, client->getFileDescriptor());
				else LOG(Logger::BULK, "client with fd %i closed connection, still %i bytes available", client->getFileDescriptor(), client->getBufferSize());
