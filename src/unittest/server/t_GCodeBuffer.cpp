...
struct t_GCodeBuffer : public fructose::test_base<t_GCodeBuffer> {
	void testSet(const std::string& test_name) {
		...
		fructose_assert_eq(buffer.getNumLines(), 0);
		buffer.set("abc");
		fructose_assert_eq(buffer.getNumLines(), 1);
	...
	void testAppend(const std::string& test_name) {
		...
		buffer.set("abc\n");
		buffer.append("xyz\nnext line");
		fructose_assert_eq(buffer.getNumLines(), 3);

		buffer.clear();
		buffer.append("def\nmore data\n");
		fructose_assert_eq(buffer.getNumLines(), 2);
	}
	...
	void testGetErase(const std::string& test_name) {
		...
		buffer.append(line1 + "\n" + line2 + "\n" + line3 + "\n" + line4);
		fructose_assert_eq(buffer.getNumLines(), 4);
	...
	void testCurrentLine(const std::string& test_name) {
		GCodeBuffer buffer;
		...
		buffer.eraseLine();
		fructose_assert_eq(buffer.getCurrentLine(), 2);
...
int main(int argc, char** argv) {
	...
	tests.add_test("currentLine", &t_GCodeBuffer::testCurrentLine);
